...
`--font-sans` 는 로컬 Pretendard를 사용한다.

## Key Files

| File | Description |
|------|-------------|
| `globals.css` | Tailwind v4 진입점. `@theme` 으로 폰트 변수 선언, `:root` 배경/전경 CSS 변수, 다크모드 `prefers-color-scheme`, 리셋 규칙 |
...
- 기본 폰트는 로컬 Pretendard를 사용한다. 컴포넌트 레벨에서 폰트 오버라이드 금지.
...
- 폰트 변수(`--font-pretendard`)는 `src/app/layout.tsx` 에서 `next/font` 가 `<html>` class 로 주입.
